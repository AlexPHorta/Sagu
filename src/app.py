...
def create_project():
    ...
    if f:=input(f"> Where do you want to create your website? [Default: '{default_settings["main_directory"]}'] ") != '':
    ...
    if f:=input(f"> What's the default language of the website? [Default: {default_settings["website_language"]}]") == '':
        default_settings["website_language"] = f

    default_settings["website_url"] = input("> What will be the website's URL "
        "(e.g. https://example.com)? [Default: empty]")
